...
const MetadataSelect: React.FC<{
  ...
  multiple?: boolean;
  type: 'namespace' | 'metric' | 'field' | 'tagKey';
  loader?: (input?: string) => any;
  placeholder?: string;
  rules?: any[];
  style?: React.CSSProperties;
}> = (props) => {
  const { loader, placeholder, labelPosition, multiple, type, rules, style } = props;
  ...
  const findMetadata = async (prefix?: string) => {
    ...
    if (type == 'tagValue') {
      ...
      const tags: string[] = URLKit.getTagConditions(params, _.get(variate, 'watch.cascade', []));
      if (!_.isEmpty(tags)) {
