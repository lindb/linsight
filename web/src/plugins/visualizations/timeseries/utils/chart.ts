...
const calcTooltipPosition = (
  ...
  switch (position) {
    ...
    case TOOLTIP_POSITION.TOP:
      ...
      containerPosition = {
        left: !reachBoundary ? mouseX - tooltipWidth / 2 : reachLeftBoundary ? TOOLTIP_SPACING : undefined,
...
export const getTooltipPositionAndSize = (
  ...
  if (bottomAreaHeight > TOOLTIP_MIN_HEIGHT || topAreaHeight > TOOLTIP_MIN_HEIGHT) {
    if (bottomAreaHeight - TOOLTIP_SPACING > tooltipHeight) {
      ...
      targetPosition = bottomAreaHeight > topAreaHeight ? TOOLTIP_POSITION.BOTTOM : TOOLTIP_POSITION.TOP;
    }
  } else {
    if (leftAreaWidth - TOOLTIP_SPACING > tooltipWidth) {
      ...
      targetPosition = leftAreaWidth > rightAreaWidth ? TOOLTIP_POSITION.LEFT : TOOLTIP_POSITION.RIGHT;
    }
  }
...
export const setPosition = (
  ...
  const topValue = top === undefined ? 'auto' : typeof top === 'number' ? `${top}px` : top;

  const rightValue = right === undefined ? 'auto' : typeof right === 'number' ? `${right}px` : right;

  const bottomValue = bottom === undefined ? 'auto' : typeof bottom === 'number' ? `${bottom}px` : bottom;

  const leftValue = left === undefined ? 'auto' : typeof left === 'number' ? `${left}px` : left;
...
export const handleSeriesClick = (chart: Chart | null, series: any, event: React.MouseEvent) => {
  const onSelect: any = _.get(chart, 'lin.extend.onSeriesSelect', null);
  if (onSelect) {
